...
sub run {
    ...
    UPDATE: foreach my $update (@updates) {
        ...
        next UPDATE if $sorted;
        say $update;

        # Setup up the indexes for the current rules
        my @rules;
        foreach my $key (sort keys %currentRules) {
            $key =~ /^(\d+)\|(\d+)$/;
            my $first = $1;
            my $second = $2;
        ...
        my $rules_index = 0;
        my %indexedRules = map {$_ => $rules_index++} @rules;
        my @sortedPages = sort {$indexedRules{$a} <=> $indexedRules{$b}} @pages;
        say join(",", @sortedPages);
        $sumMiddlePages += getMiddlePage(@sortedPages);
    }
    say $sumMiddlePages;
}
